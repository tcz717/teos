...
#if defined(__is_teos_kernel)
int tputchar(int ic);
static void tprint(const char* data, size_t data_length)
{
	for ( size_t i = 0; i < data_length; i++ )
		tputchar((int) ((const unsigned char*) data)[i]);
	
	terminal_updatecurser();
}
// This is temporary solution. And once heap finshed, 
// we would change code to call sprintf.
